...
class EventFetcher {
  ...
  async _format(logs) {
    ...
    for(const log of logs) {
      blockHashMap.set(log.blockHash, log.blockNumber);
    }
  ...
  async _getBlocks(blockHashes, blockHashMap) {
    ...
    if (blocks.indexOf(null) !== -1) {
      blocks = await Promise.all(uncachedBlockHashes.map((e) => {
        const blockNumber = blockHashMap.get(e);
        limit(this._web3.eth.getBlock, blockNumber, true)
      }));
    }
