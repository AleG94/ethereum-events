...
class EventFetcher {
  constructor(web3, contracts, options = {}) {
    ...
    this._ignoreUnknownEvents = options.ignoreUnknownEvents || config.ignoreUnknownEvents;
  ...
  _parse(logs) {
    ...
    for (const log of logs) {
      try {
        parsedLogs.push(
          parseLog(log, this._contracts.find(c => c.address.toLowerCase() === log.address.toLowerCase()).abi)
        );
      } catch (err) {
        if (err.message !== 'Cannot parse unknown event' || !this._ignoreUnknownEvents) {
          throw err;
        }
      }
    }
