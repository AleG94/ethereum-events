...
class EventFetcher {
  constructor(web3, contracts, options = {}) {
    ...
    this._partialAbi = options.partialAbi || false;
  }
  ...
  _filter(logs) {
    return logs.filter((log) => {
      const contract = this._contracts.find(
        (c) => c.address.toLowerCase() === log.address.toLowerCase()
      );
  ...
  async _format(logs) {
    ...
    const blockHashMap = logs.map((log) => {
      return { blockHash: log.blockHash, blockNumber: log.blockNumber }
    });
    const blocks = await this._getBlocks(blockHashes, blockHashMap);
  ...
  async _getBlocks(blockHashes, blockHashMap) {
    const results = [];
    ...
    const blocks = await Promise.all(
      uncachedBlockHashes.map((e) => limit(this._web3.eth.getBlock, e, true))
    );

    /** Workaround for viction */
    if (blocks.indexOf(null) !== -1) {
      blocks = await Promise.all(
        uncachedBlockHashes.map((e) => {
          const blockNumber = blockHashMap.find(element => element.blockHash === e) || -1;
          return limit(this._web3.eth.getBlock, blockNumber.blockNumber, true);
        })
      );
      blocks.forEach(block => {
        const logHash = blockHashMap.find(element => element.blockNumber === block.number);
        block.hash = logHash.blockHash;
      });
    }
    /** Workaround for viction */
