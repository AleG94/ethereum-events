...
class BlockPolling {
  constructor(web3, eventFetcher, options = {}) {
    ...
    this._confirmations = options.confirmations !== undefined ? options.confirmations : config.confirmations;
    this._backoff = options.backoff || config.backoff;
    this._eventCache = safeMemoryCache({ limit: Math.max(1, this._confirmations) });
  }
